...
Both `llm` and `bot` carry `restart: unless-stopped`, so after a PC reboot the Docker daemon brings the whole rig back by itself: token and access re-read from the container env, poll offset and per-chat sessions from `./data/`. It resumes listening with no human involved; `docker compose logs -f bot` to peek.
